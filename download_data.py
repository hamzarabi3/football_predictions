...
Other_Leagues = pd.concat(
    [MLS2020, Swed2020, Nor2020, LigaMx2020, Arg, AUT, BRA, CHN, FIN]
)

Other_Leagues.Date = pd.to_datetime(Other_Leagues.Date)
Other_Leagues = Other_Leagues.rename(
    columns={
        "Date": "date",
        "Home": "home_team",
        "Away": "away_team",
...
sets = [set(dict_historical_data[l].columns) for l in leagues] + [
    set(Other_Leagues.columns)
]
