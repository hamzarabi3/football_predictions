...
def run_inference(lookback=5, results_add_up=False):
    ...
    for x in range(1, lookback + 1):
        ...
        league[new_columnh] = league.apply(
            lambda raw: get_previous_match_result(
                x, raw.home_team, raw.date, history_df
            ),
            axis=1,
        )
        ...
        league[new_columna] = league.apply(
            lambda raw: get_previous_match_result(
                x, raw.away_team, raw.date, history_df
            ),
            axis=1,
        )
    ...
    for x in range(1, lookback + 1):
        ...
        print(".", end="\t")
        league[new_columnh] = league.apply(
            lambda raw: get_previous_match_goals(
                x, raw.home_team, raw.date, history_df
            ),
            axis=1,
        )
    ...
    categoricals_df = pd.DataFrame(
        ohe.transform(categoricals_df), index=categoricals_df.index
    )
